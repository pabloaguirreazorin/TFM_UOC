...
rankings_ad_nci <- sign(top_proteomica_ad_nci$logFC) * 
...
```{r}
# Calculo los rankings
rankings_ad_mci <- sign(top_proteomica_ad_mci$logFC) * 
...
write.table(as.data.frame(gsea_go_ad_mci), "E:\\Documentos\\UNIVERSIDAD\\UOC\\TFM\\_R\\proteomica\\GSEA_AD_MCI\\GSEA_proteomica_ad_mci.csv")
```
...
# Dotplot
# library(DOSE)
dotplot(gsea_go_ad_mci, showCategory=10, split=".sign") + facet_grid(.~.sign) +
  theme(axis.title = element_text(size=12, face="bold"),
        axis.title.x = element_text(margin = margin(t=20, r=0, b=0, l=0)),    
        strip.text = element_text(face="bold", size=9), 
        axis.text.x = element_text(size=8),
...
```{r}
# Calculo los foldchanges de los genes de la rank list
foldChanges_ad_mci <- top_proteomica_ad_mci[match(names(rankings_ad_mci),
                  rownames(top_proteomica_ad_mci)),]$logFC
...
```{r}
# Calculo los rankings
rankings_mci_nci <- sign(top_proteomica_mci_nci$logFC) * 
...
write.table(as.data.frame(gsea_go_mci_nci), "E:\\Documentos\\UNIVERSIDAD\\UOC\\TFM\\_R\\proteomica\\GSEA_MCI_NCI\\GSEA_proteomica_mci_nci.csv")
```
...
```{r}
# Calculo los foldchanges de los genes de la rank list
foldChanges_mci_nci <- top_proteomica_mci_nci[match(names(rankings_mci_nci), rownames(top_proteomica_mci_nci)),]$logFC
...
proteomic_topvarProteinas <- dge_normalizado[topvarProteinas,]
```
