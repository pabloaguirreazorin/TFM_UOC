...
Antes de comenzar, voy a sumar un pequeño valor si el valor beta es 0 y restárselo si el valor beta es 1. Esta manera de hacerlo no es lo habitual, pero lo hago porque al calcular los valores M mediante la función `getM` de `minfi`, no me permite añadir un `betaThreshold`.

```{r}
datos_metilacion[datos_metilacion == 0] <- 0.00001
datos_metilacion[datos_metilacion == 1] <- 0.99999
```
...
mVals <- getM(metilacion_grs_filtrado) 
...
## Exportación de la tabla de valores M (REVISAR)

Exporto la tabla de valores M para realizar la integración multiómica.

```{r}
write.table(mVals, "E:\\Documentos\\UNIVERSIDAD\\UOC\\TFM\\_R\\integracion_multiomica\\mvals_metilacion.csv")
```
