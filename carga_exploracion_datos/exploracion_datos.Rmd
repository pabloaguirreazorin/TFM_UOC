...
De la tabla pacientes voy a seleccionar las variables `individualID`, `msex`, `cogdx` (correspondiente al diagnóstico final en el momento de la muerte del paciente), `educ`, `race`, `spanish` y `apoe_genotype`.
...
pacientes <- pacientes %>% 
  mutate(msex = factor(msex, labels = c("Female", "Male")),
         ...
         cogdx = as.factor(case_when(cogdx == 1 ~ "NCI",
                                     ...
                                     cogdx %in% c(4,5) ~ "AD",
                                     cogdx == 6 ~ "Other"))) %>% 
  select(individualID, msex, cogdx, educ, race, spanish, apoe_genotype)
```
...
contajes_rnaseq_raw_genes <- contajes_rnaseq_raw %>% 
...
muestras_rnaseq <- colnames(contajes_rnaseq_raw_genes)
...
```{r}
# Gráfico resumen de los datos filtrados

pacientes_rnaseq %>% 
  group_by(cogdx, msex) %>% 
  summarise(Cantidad = n()) %>% 
  ggplot(aes(x = cogdx, y = Cantidad, fill = msex)) +
  geom_col() +
  geom_label(aes(label = Cantidad), position = position_stack(vjust = 0.5),
             show.legend = FALSE) +
  labs(fill = "Grupo diagnóstico", 
...
colnames(contajes_rnaseq_raw_genes) <- muestras_rnaseq_id

# Elimino las muestras cuyo diagnóstico es "Other" y de las que no consta su paciente asociado.
contajes_rnaseq_raw_genes_filtrado <- contajes_rnaseq_raw_genes %>% 
...
```{r}
write.table(contajes_rnaseq_raw_genes_filtrado,
...
pacientes_proteinas_sinfiltrar <- pacientes %>% 
  filter(individualID %in% muestras$individualID[muestras$specimenID
    ...
    rowwise() %>%  # Para que la siguiente operación se produzca fila por fila
  mutate(proteomicID = metadatos_proteomicos$SampleID[metadatos_proteomicos$SpecimenID %in% 
                       muestras$specimenID[muestras$individualID == individualID]],
         batch = metadatos_proteomicos$Batch[metadatos_proteomicos$SpecimenID %in% 
                       muestras$specimenID[muestras$individualID == individualID]])
```
